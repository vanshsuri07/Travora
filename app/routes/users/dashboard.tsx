// Create this as a new route file (e.g., routes/users/dashboard.tsx)
...
export const clientLoader = async () => {
    try {
        ...
        // Format trips data to match your UpcomingTrips component structure
        const userTrips = userTripsResponse.userTrips.map(({ $id, tripDetails, imageUrls }) => ({
            $id,
            id: $id,
...
const UserDashboard = ({ loaderData }: Route.ComponentProps) => {
    const { user, userTrips, totalTrips } = loaderData;

    // Function to refetch user trips (for the refresh functionality)
    const fetchUserTrips = async () => {
        try {
            const tripsResponse = await getUserTrips(user.$id, 10, 0);
            return tripsResponse.userTrips.map(({ $id, tripDetails, imageUrls }) => ({
                $id,
                id: $id,
                ...parseTripData(tripDetails),
                imageUrls: imageUrls ?? []
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-gray-600">
                        {totalTrips > 0 
                            ? `You have ${totalTrips} trip${totalTrips !== 1 ? 's' : ''} planned`
                ...
                {totalTrips > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-sm border">
                            <h3 className="text-sm font-medium text-gray-500">Recent Trips</h3>
                            <p className="text-2xl font-bold text-gray-900">{Math.min(userTrips.length, 5)}</p>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow-sm border">
                            ...
                            <p className="text-2xl font-bold text-gray-900">
                                {userTrips.filter(trip => {
                                    ...
                                    return tripDate.getMonth() === thisMonth.getMonth() && 
                ...
                <div className="mb-8">
                    <h2 className="text-2xl font-bold text-gray-900 mb-6">Your Upcoming Trips</h2>
                    <UpcomingTrips 
                        trips={userTrips}
