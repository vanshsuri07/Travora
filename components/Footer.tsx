      ...
      <div className="max-w-7xl mx-auto py-12 px-6 sm:px-8 lg:px-12">
        <div className="xl:grid xl:grid-cols-3 xl:gap-8">
          <div className="space-y-8 xl:col-span-1">
            <h2 className="text-2xl font-bold text-white">Travora</h2>
            <p className="text-gray-400 text-base">
              Discover amazing destinations and create unforgettable memories with our curated travel packages.
            </p>
            <div className="flex space-x-6">
              {socialLinks.map((item) => (
                <a key={item.name} href={item.href} className="text-gray-400 hover:text-cyan-400 transition-colors duration-300">
              ...
              <div>
                ...
                <ul className="mt-4 space-y-4">
                  {footerLinks.map((link) => (
                     <li key={link.title}><a href={link.href} className="text-base text-gray-400 hover:text-white">{link.title}</a></li>
                  ))}
                </ul>
              </div>
        ...
        <div className="mt-12 border-t border-gray-700 pt-8">
          <p className="text-base text-gray-400 xl:text-center">&copy; {new Date().getFullYear()} Travora, Inc. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
